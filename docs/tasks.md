...
- Trạng thái: **Đã triển khai (Backend + Frontend)**.
...
- Trạng thái: **Đã triển khai phần lớn (Backend + Frontend)**.
- Ghi chú tiến trình:
  - B17 đã có trang cảnh báo theo tuần (`weekOffset`) và badge thiếu người trên tổng quan manager.
  - B26 đã dùng đúng endpoint báo cáo theo tuần/tháng.
  - B24/B25 đã có UI cấu hình lương và bảng lương; đang tinh chỉnh trải nghiệm hiển thị “lương dự kiến” cho staff ở trang tổng quan.
...
1. **\[B08] Khai báo Lịch rảnh**: API lưu Availability theo tuần.
   ...
   1. **\[B11] Xem Ca phù hợp**: Logic filter ca `OPEN` + `Availability` + chưa full slot.
      - *Branch*: `feature/member-view-shifts`
      - *Commits*: `feat(shift): add filter for available shifts`
2. **\[B12] Đăng ký Ca**: API tạo Registration (`PENDING`), check validate cơ bản.
...
3. **\[B13] Hủy Đăng ký**: API hủy đăng ký (check rule thời gian).
   - *Branch*: `feature/member-cancel-reg`
   - *Commits*: `feat(reg): add cancel registration logic`
4. **\[B19] Xem Lịch cá nhân**: API lấy danh sách ca đã `APPROVED` của user.
...
| User Governance        | `GET /api/v1/admin/users`, `PATCH /users/{id}/toggle-status`   | `feature/admin-users`            | Lọc/phân trang đúng; bật/tắt user đúng trạng thái           |
| Group Governance       | `GET /api/v1/admin/groups`, `PATCH /groups/{id}/toggle-status` | `feature/admin-groups`           | Lọc/phân trang đúng; bật/tắt group đúng trạng thái          |
| Admin Metrics          | `GET /api/v1/admin/metrics`                              | `feature/admin-metrics`          | Dashboard hiển thị đủ số liệu tổng quan hệ thống            |
